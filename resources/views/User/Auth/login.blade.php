...
<head>
    ...
    <title>{{ $title ?? 'CareerNest' }} -  @yield('title') Login</title>
...
<body>
    ...
    <script>
        $('#Login').on('submit', function(e) {
            ...
            // Manually append remember checkbox value
            formData.append('remember', $('#flexCheckDefault').is(':checked') ? 1 : 0);

            $.ajax({
                url: "{{ route('User.loginInsert') }}",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                ...
                success: function(result) {
                    if (result.status_code == 1) {
                        ...
                        Toastify({
                            ...
                            gravity: "top",
                            position: "right",
                            className: result.status_code == 2 ? "bg-warning" : "bg-danger"
                        }).showToast();
                    }
                }
            });
