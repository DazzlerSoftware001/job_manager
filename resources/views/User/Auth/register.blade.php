...
<head>
    ...
    <title>{{ $title ?? 'CareerNest' }} -  @yield('title') Register</title>
        ...
        <div class="card shadow rounded-4 border-0" style="max-width: 500px; width: 100%;">
            <div class="card-body p-4">
                ...
                <form action="javascript:void(0)" method="post" id="Register" class="d-flex flex-column gap-3">
                    ...
                    <div class="form-group">
                        <label for="password_confirmation" class="fw-medium text-dark mb-2">Confirm Password</label>
                        <div class="position-relative">
                            <input type="password" name="password_confirmation" id="password_confirmation"
                                class="form-control" placeholder="Confirm your password" required>
                            <i
                                class="fa-light fa-lock icon position-absolute end-0 top-50 translate-middle-y me-3"></i>
                        </div>
                    </div>
                ...
                success: function(result) {
                    if (result.status_code == 1) {
                        ...
                        Swal.fire({
                            ...
                            preConfirm: (otp) => {
                                return $.ajax({
                                    url: "{{ route('User.verifyOtp') }}", // route to verify OTP
                                    method: 'POST',
                                    headers: {
                                    ...
                                    data: {
                                        email: $('#email')
                                            .val(), // assuming your form has #email field
                                        otp: otp
                                    }
                                }).then(response => {
