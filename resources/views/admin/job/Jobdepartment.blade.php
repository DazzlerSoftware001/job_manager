...
@section('title')
    Admin-Job Department
@endsection
@section('page-title')
     <div class="breadcrumb mt-2">
        {!! Breadcrumbs::render('Admin.JobDepartment') !!}
    </div>
...
@section('main-container')
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Job Department</h4>
                            </div>
                            ...
                            <div class="px-3 mt-3 d-flex align-items-center gap-2">
                                ...
                                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModal">+ Add Job Department</button>
                            </div>
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <h1 class="modal-title fs-5" id="exampleModalLabel">Add job department</h1>
    ...
    <div class="modal fade" id="EditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header">
            <h1 class="modal-title fs-5" id="exampleModalLabel">Edit job department</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <!-- form -->
                <form method="POST" action="javascript:void(0)" id="EditJobDepartment">
...
@section('script')
    ...
    <script>
        ...
        $('#myTable').DataTable({
            ...
            "order": [[0, "desc"]],
            "ajax": {
                url: "{{ route('Admin.GetJobDepartment') }}",
                type: 'POST',
                headers: {
    ...
    <script>
        $(document).ready(function() {
            $('#AddJobDepartment').on('submit', function(event) {
                event.preventDefault(); // Prevent default form submission
        
                var url = "{{ route('Admin.AddJobDepartment') }}";
                $.ajax({
                    url: url,
                    type: 'POST',
                    headers: {
                    ...
                    success: function(result) {
                        if (result.status_code === 1) {
                            ...
                            $('#AddJobDepartment').trigger("reset");
            ...
            function changeStatus(id) {
                Swal.fire({
                    ...
                    if (response.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('Admin.ChangeJobDepartmentStatus') }}",
                            type: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                        ...
                        Swal.fire({
                            title: 'Cancelled',
                            text: 'The department status was not changed.',
                            icon: 'info',
            ...
            Swal.fire({
                ...
                if (response.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('Admin.DeleteJobDepartment') }}",
                        data: { id: id },
                        dataType: 'json',
    ...
    <script>
        ...
        $.ajax({
            ...
            url: "{{ route('Admin.EditJobDepartment') }}",
            data:{ id:id },
            dataType:'json',
            success:function(result)
            {   
    ...
    <script>
    $(document).ready(function() {
        $('#EditJobDepartment').on('submit', function(event) {
            event.preventDefault(); // Prevent default form submission
    
            var url = "{{ route('Admin.UpdateJobDepartment') }}";
            $.ajax({
                url: url,
                type: 'POST',
                headers: {
                ...
                success: function(result) {
                    if (result.status_code === 1) {
                        ...
                        $('#EditJobDepartment').trigger("reset");
                        $('#myTable').DataTable().ajax.reload(null, false);
                        Toastify({
                            text: result.message,
                            duration: 3000,
