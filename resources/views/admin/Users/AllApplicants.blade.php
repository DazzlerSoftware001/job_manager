...
@section('page-title')
    <div class="breadcrumb mt-2">
        {!! Breadcrumbs::render('Admin.AllApplicants') !!}
    </div>
                        ...
                        <div class="card">
                            ...
                            <div class="row mt-2 px-3">
                                ...
                                <form id="exportForm" method="POST"
                                    action="{{ route('Admin.FilteredApplicantsExportExcel') }}">
                                    @csrf
                                    <input type="hidden" name="recruiter" id="export_recruiter">
                                    <input type="hidden" name="job" id="export_job">
    ...
    @section('script')
                    ...
                    if (recruiterId) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                $('#JobFilter').empty().append(
                                    '<option value="">Filter by Job</option>');
                                $.each(data, function(key, job) {
                                    $('#JobFilter').append('<option value="' + job.id +
                                        '">' + job.title + '</option>');
                                });
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#JobFilter, #education_level, #Qualification, #Branch, #cityFilter,#skills, input[name="experience[]"]')
                    .on('change', function() {
        ...
        <script>
            $(document).ready(function() {
                $('#exportBtn').on('click', function() {
                    $('#export_recruiter').val($('#RecruiterFilter').val());
                    $('#export_job').val($('#JobFilter').val());
