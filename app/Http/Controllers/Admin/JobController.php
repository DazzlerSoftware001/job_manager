    ...
    public function submitJob(Request $request)
    {
        ...
        if (! $validator->fails()) {
            try {
                ...
                $template = EmailTemplates::find(5);

                if ($template && $template->show_email == '1') {
                    try {
                        $recruiter = UserProfile::where('id', $request->input('recruiter_id'))
                            ->select('name', 'lname', 'email')
                            ->first();
                        Mail::to($recruiter->email)->send(new JobPostMailToRecruiter($JobPost, $recruiter));
                    } catch (\Exception $mailException) {
                        DB::rollBack();
                        Log::error('Job Post email send failed: ' . $mailException->getMessage());
                        return response()->json([
                            'status_code' => 0,
                            'message'     => 'Job Post creation failed while sending email.',
                        ]);
                    }
                }

                $message = ($template && $template->show_email == '1') ?
                'Job Post Successfully and Email Sent.' :
                ...
                DB::rollBack();
                Log::error('Job Post creation error: ' . $e->getMessage());
                // Handle any exception that occurs during saving
                ...
                if ($JobPost->save()) {
                    ...
                    if ($template && $template->show_email == '1') {
                        try {
                            ...
                            return response()->json([
                                'status_code' => 0,
                                'message'     => 'Job Post failed while sending email.',
                            ]);
                        }
                    }

                    $message = ($template && $template->show_email == '1') ?
                    'Job Post Update and email sent successfully.' :
